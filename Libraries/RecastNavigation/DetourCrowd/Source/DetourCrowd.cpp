...
static float getDistanceToGoal(const dtCrowdAgent* ag, const float range)
{
	...
	if (endOfPath)
	{
		float dist = dtVdist2D(ag->npos, &ag->cornerVerts[(ag->ncorners-1)*3]);
		return dtMax(-range, dtMin(range, dist));
	}
...
void dtCrowd::updateAgentState(int idx, unsigned char state)
{
	m_agents[idx].state = state;
}
...
void dtCrowd::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		{
			ag->boundary.update(ag->corridor.getFirstPoly(), 
								ag->npos, 
		...
		if (ag->params.updateFlags & (
				...
				DT_CROWD_COLLISION_RESOLUTION))
		{
			ag->nneis = getNeighbours(ag->npos, 
									  ag->params.height, 
									  ag->params.collisionQueryRange,
									  ag, 
									  ag->neis, 
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		const float slowDownRadius = ag->params.radius;	// TODO: make less hacky (still).
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (ag->params.updateFlags & DT_CROWD_OBSTACLE_AVOIDANCE)
		{
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				float mult;
				if (ag->params.updateFlags & DT_CROWD_AGENT_PRIORITIES)
				{
					mult = (nei < ag) ? 0.5f : 2.0f;
				}
				else
				{
					mult = 1.0f;
				}
				m_obstacleQuery->addCircle(nei->npos, 
										   nei->params.radius, 
										   nei->vel, 
										   nei->dvel,
										   mult);
	...
	for (int iter = 0; iter < 4; ++iter)
	{
		for (int i = 0; i < nagents; ++i)
		{
			...
			if (!(ag->params.updateFlags & DT_CROWD_COLLISION_RESOLUTION))
				continue;
			...
			for (int j = 0; j < ag->nneis; ++j)
			{
				...
				if(ag->params.updateFlags & DT_CROWD_AGENT_PRIORITIES)
				{
					if(ag < nei)
					{
						dtVmad(ag->disp, ag->disp, diff, pen*2);
					}
				}
				else
				{
